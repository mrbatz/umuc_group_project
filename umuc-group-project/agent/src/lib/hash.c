...
extern void sha256_transform(SHA256_CTX_t *ctx, const BYTE data[])
{
...
extern int sha256sum_file(char filepath[MAX_PATH_LENGTH], unsigned char hash[SHA256_LENGTH])
{
    ...
    {
        fclose(f);
        log(CRIT, "Failed to allocate enough memory!");
        return 1;
    }
    ...
    while((bytesRead = fread(buffer, 1, bufSize, f)))
    {
